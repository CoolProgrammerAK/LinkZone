import { SIGNUP_ERROR, SET_USER,SIGNOUT_SUCCESS,LOGIN_ERROR,LOGIN_SUCCESS,SIGNUP_SUCCESS,LOADING_UI,IMAGE_ERROR, IMAGE_SUCCESS, THEME_UPDATE, THEME_LOADER, RESET_LOADER, RESET_UPDATE,} from '../type/type'
import {showtoast} from '../../toast/toast'
 

export const signUp=(data,history)=>(dispatch,getState,{getFirebase,getFirestore})=>{
...
export const logIn=(data,history)=>(dispatch,getState,{getFirebase})=>{
  dispatch({type:LOADING_UI})
    const firebase=getFirebase()
    ...
    export const logInwithgoogle=(history)=>(dispatch,getState,{getFirestore,getFirebase})=>{
       ...
       firestore.collection('users').doc(res.user.uid).set({
        ...
        export const logInwithfacebook=(history)=>(dispatch,getState,{getFirestore,getFirebase})=>{
 ...
 export const uploadimage=image=>(dispatch,getState,{getFirestore,getFirebase})=>{
   const firestore=getFirestore()
   const firebase=getFirebase()
  ...
  upload.on(
     'state_changed',snapshot=>{},error=>{
       dispatch({type:IMAGE_ERROR,action:error})
     },
     ()=>{
      firebase.storage().ref(userhandle).getDownloadURL().then(url=>
 ...
 export const updatetheme=(theme)=>(dispatch,getState,{getFirestore})=>{
  const firestore=getFirestore()
  const uid=getState().firebase.profile.uid
...
export const reset=(email)=>(dispatch,getState,{getFirestore,getFirebase})=>{
